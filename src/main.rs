...
async fn main() {
    ...
    for (k, v) in default_settings {
        let _ = sqlx::query("INSERT OR IGNORE INTO system_settings (key, value) VALUES (?, ?)")
            .bind(k).bind(v).execute(&db).await;
    }

    // Ignore error if columns already exist
    let _ = sqlx::query("ALTER TABLE scan_results ADD COLUMN port INTEGER DEFAULT 443").execute(&db).await;
    let _ = sqlx::query("ALTER TABLE scan_history ADD COLUMN total_tasks INTEGER DEFAULT 0").execute(&db).await;
    let _ = sqlx::query("ALTER TABLE scan_history ADD COLUMN completed_tasks INTEGER DEFAULT 0").execute(&db).await;
    let _ = sqlx::query("ALTER TABLE scan_history ADD COLUMN status TEXT DEFAULT 'COMPLETED'").execute(&db).await;
    ...
    let app = Router::new()
        ...
        .route("/results/:ip", axum::routing::delete(handlers::delete_result_handler))
        .route("/settings", get(handlers::get_settings_handler).put(handlers::put_settings_handler))
        .with_state(state);
...
async fn run_health_check(db: sqlx::sqlite::SqlitePool) {
    let rows: Vec<(String, i64, String)> = sqlx::query_as("SELECT ip, port, origin FROM scan_results WHERE feasible = true")
    ...
    let iter = rows.into_iter().map(|(ip_str, port, origin)| {
        ...
        async move {
            if let Ok(ip) = IpAddr::from_str(&ip_str) {
                ...
                if !res.feasible {
                    let _ = sqlx::query("UPDATE scan_results SET feasible = false WHERE ip = ? AND port = ?")
...
async fn ensure_geo_db() {
    ...
    if !std::path::Path::new(db_path).exists() {
        ...
        match reqwest::get("https://github.com/Loyalsoldier/geoip/releases/latest/download/Country.mmdb").await {
            Ok(response) => {
                if response.status().is_success() {
                    ...
                    println!("Failed to download GeoIP Database, status code: {}", response.status());
                }
            }
            Err(e) => {
